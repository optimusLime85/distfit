...
def sampleWithReplacement(obs, prob, nSim=1E4):
    ...
    indRand = np.random.choice(ind, size=nSim, replace=True,
                               p=prob)  # randomly pick from ind with probabilities per entry of prob
...
def plotSummary(reportedSize, nonReportedSample, dist, distributionType, fitType, xTitle, regionID, outPath):
    ...
    binsDist = np.linspace(np.min(reportedSize) - 0 * (np.max(reportedSize) - np.min(reportedSize)),
                           np.max(reportedSize) + 0.1 * (np.max(reportedSize) - np.min(reportedSize)), 300)
    distPDF = dist.pdf(binsDist)

    plt.hist([reportedSize, nonReportedSample], binsHist, normed=True, color=[lineColors[0], lineColors[1]],
             edgecolor="none", rwidth=0.85, label=['Reported', 'Unreported'], alpha=0.5)
    ...
    plt.savefig(outPath + '\\' + regionID + '_' + distributionType + '_' + fitType + '_' + 'summary' + '.png', dpi=300,
                format='png')
    return


def processRegion(reportedSize, relativeFrequency, distributionType, minAtZero, defectDimension, plotUnits, regionID,
                  outPath, showPlots, useData):
    ...
    for s in [np.mean(nonReportedSample), np.std(nonReportedSample, ddof=1),
              np.min(nonReportedSample)]: summaryList += str(s) + ','
    ...
    for fitType in fitTypes:
        ...
        if isConverged:
            [sqrErr, mean, stdDev, lowerBound] = dfit.probPlotSqrErr(nonReportedSample, dist, distributionType, xTitle,
                                                                     fitType, regionID, outPath,
            ...
            print('data:', np.mean(nonReportedSample), np.std(nonReportedSample, ddof=1), np.min(nonReportedSample),
                  sep='\t')
            if showPlots:
...
def main(currPath, inputFileName, showPlots):
    ...
    depthWriteList.append(
        ',' * 5 + 'Simulated Sample' + ',' * 3 + 'MLE' + ',' * 4 + 'Quantiles' + ',' * 4 + 'MOM' + ',' * 4 + '\n')
    paramHeaders = 'Mean' + ' (' + defectUnits + ')' + ',' + 'Std Dev' + ' (' + defectUnits + ')' + ',' + 'Lower Bound' + ' (' + defectUnits + ')'
    depthWriteList.append(
        'Region Number' + ',' + 'Start' + ' (' + postUnits + ')' + ',' + 'End' + ' (' + postUnits + ')' + ',' + \
    ...
    for iRegion in range(len(regionStart)):
        ...
        # get the likelihood of non-reported defects for each reported defect; the relative frequency and the total expected number of non reported defects
        [nNonReported, likelihood, relativeFrequency] = nonReportedParams(reportedDepth, detThreshold, PODthreshold,
                                                                          POFC, POI, useConstPOD, constPOD)
        ...
        for s in [regionNums[iRegion], regionStart[iRegion], regionEnd[iRegion], regionLength,
                  defectDensity]: depthWriteList.append(str(s) + ',')
        if processLength:
            for s in [regionNums[iRegion], regionStart[iRegion], regionEnd[iRegion], regionLength,
                      defectDensity]: lengthWriteList.append(str(s) + ',')
        if processWidth:
            for s in [regionNums[iRegion], regionStart[iRegion], regionEnd[iRegion], regionLength,
                      defectDensity]: widthWriteList.append(str(s) + ',')

        if nReported > 1:  # need at least 2 defects in this region

            defectDimension = 'Depth'
            summaryList = processRegion(reportedDepth, relativeFrequency, distributionType, minAtZero, defectDimension,
                                        defectUnits, regionID + '_' + defectDimension, outPath, showPlots, useData)
            ...
            if processLength:
                ...
                summaryList = processRegion(reportedLength, relativeFrequency, distributionType, minAtZero,
                                            defectDimension, defectUnits, regionID + '_' + defectDimension, outPath,
                                            showPlots, useData)
            ...
            if processWidth:
                ...
                summaryList = processRegion(reportedWidth, relativeFrequency, distributionType, minAtZero,
                                            defectDimension, defectUnits, regionID + '_' + defectDimension, outPath,
                                            showPlots, useData)
