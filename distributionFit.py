...
class Distribution:
    ...
    def initParams(self, data):
        ...
        if self.type == 'normal':
            ...
            if self.mode[1][0] != 1.0: # we can only use the mode for the distribution shape if all values are not unique
        ...
        if fit == 'MLE':
            ...
            def objFunc(X):
                ...
                if fit == 'quantiles':
                    ...
                    weights = [1, 1, 0.1] # scale the influence of each moment # set last entry to remove skewness from the assessment
                    ...
                    obj = np.sum([abs(dataMoments[i]-distMoments[i])/max(dataMoments[i], 1E-6)*weights[i]  for i in range(self.nParams)]) # only use the number of moments needed to specify the distribution to match the data # np.sum((distMoments-dataMoments)**2) # np.sum([abs( (dataMoments[i]-distMoments[i])**(1/(i+1)) ) for i in range(3)]) #np.sum((dist.moments()-dataMoments)**2)
                return obj
            
            
            X = [self.shape, self.loc, self.scale]
            
            res = minimize(objFunc, X, method = 'SLSQP', options={'disp': True, 'maxiter': 600, 'ftol': 1e-8}) #, bounds=bnds, constraints=cons, # options={'maxiter': 500, 'gtol': 1e-6, 'disp': True} 
...
def probPlotSqrErr(data, dist, distributionType, xTitle = 'Value', fitType = 'defaults', runID='', outPath='', showPlots=True):
    """ 
    ...
    if showPlots:
        ...
        binsDist = np.linspace(np.min(data) - 0.1*(np.max(data)-np.min(data)), np.max(data) + 0.1*(np.max(data)-np.min(data)), 300) 
        ...
        plt.grid(False)
        plt.savefig(outPath + '\\' + runID + '_' + distributionType + '_' + fitType + '_' + 'histogram' + '.png', dpi = 300, format = 'png')
...
#         plotPercentiles = plotPercentiles[max(iStart-1, 0):min(iEnd+1, len(plotPercentiles)-1)] # limit the range of the plot to the closest pair of points to bracket the data percentiles 
        ...
        ax.set_ylim([min(yLimCurr[0], yAxisMin), max(yLimCurr[1], yAxisMax)]) # reset back to limits determined by the range of the data unless it needs to go wider to get to yAxisMin or yAxisMax
        
        plt.xlabel(distributionType + ' ' + 'Data Quantiles')
        plt.ylabel(distributionType + ' ' + 'Theoretical Quantiles')
        
        
        plt.savefig(outPath + '\\' + runID + '_' + distributionType + '_' + fitType + '_' + 'QQ' + '.png', dpi = 300, format = 'png')
        ...
        ax.set_xlim(xLimCurr) # reset back to limits determined by the range of the data
        ax.set_ylim([min( min(dataTheorQuantiles), yAxisMin ), max( max(dataTheorQuantiles), yAxisMax )]) # reset back to limits determined by the range of the data unless it needs to go wider to get to yAxisMax and yAxisMax
        
        plt.savefig(outPath + '\\' + runID + '_' + distributionType + '_' + fitType + '_' + 'QQcdf' + '.png', dpi = 300, format = 'png')
        ...
        ax.set_yticks(distTheorQuantilesPlot[1:-1])
        ax.set_yticklabels(plotPercentiles[1:-1])
        ax.set_xlim([max(xLimCurr[0], logXaxisMin), xLimCurr[1]]) # reset back to limits determined by the range of the data
        ax.set_ylim([max( min( min(dataTheorQuantiles), yAxisMin ), logYaxisMin ), max( max(dataTheorQuantiles), yAxisMax )]) # reset back to limits determined by the range of the data unless it needs to go wider to get to yAxisMax - do not stretch below yAxisMin because skew distributions can go very low for x values close to Xshift
        
        plt.grid(False, which='minor', axis='both') #, linestyle='-', color='lightgrey')
        
        plt.savefig(outPath + '\\' + runID + '_' + distributionType + '_' + fitType + '_' + 'QQcdf_loglog' + '.png', dpi = 300, format = 'png')
...
def main(currPath, inputFileName, showPlots):
    ...
    [sqrErr, mean, stdDev, lowerBound] = probPlotSqrErr(data, dist, distributionType, xTitle, 'MLE', runID, outPath, showPlots) # output: [sqrErr, mean, stdDev, lowerBound]
    ...
    if isConverged:
        [sqrErr, mean, stdDev, lowerBound] = probPlotSqrErr(data, dist, distributionType, xTitle, fitType, runID, outPath, showPlots) # output: [sqrErr, mean, stdDev, lowerBound]
        print(sqrErr, mean, stdDev**2, lowerBound)
        print('dist.moments():          ', dist.moments())
        print('dist central moments:    ', dist.distObj.stats(moments='mvs'))
        print('data central moments:    ',stats.moment(data, moment=1), stats.moment(data, moment=2), stats.moment(data, moment=3))
